...
class McpRegression():
    ...
    def fit(self, X, y, values=None, verbose=False, mode=None, bitmask=None):
        ...
        # mode[0] = '1', int quantization
        # mode[1] = '1', v-scaled penalty
        # mode[1] = '2', log(v)-scaled penalty
        # mode[1] = '3', if (v>1) set w to 0

        self.values_full = values
        self.mode = mode
        # N = ncycles, D = nsigs
        N, D = X.shape 
        ...
        print ('alpha, gamma, max_iter, positive, mode', self.alpha, self.gamma, self.max_iter, self.positive, mode)
        print ('ncycles, nsigs, tog_sigs', N, D, D_tog)

        norm = norm_full[self.nz_mask]
        self.values = self.values_full[self.nz_mask]
        ...
        assert len(self.values) == len(self.W)
        assert len(self.values) == len(norm)
        ...
        for i in range(self.max_iter):
            ...
            if i > 0 and verbose:
                ...
                print ('Bits', np.sum(self.values[self.W != 0]) )
            ...
            for j in range(D_tog):
                ...
                if self.mode[1] == '1':
                    self.W[j] = self.mcp_v(rho, v) / norm[j]
                elif self.mode[1] == '2':
                    v = math.log2(v) + 1
                    self.W[j] = self.mcp_v(rho, v) / norm[j]
                elif self.mode[1] == '3':
                ...
                if self.mode[0] == '1':
                    #self.W[j] = int(self.W[j])
                    self.W[j] = round(self.W[j])
    ...
    def mcp_v(self, wj, v):
        if abs(wj) >= self.gamma * self.alpha * v:
            if wj > 0:
                return wj
            else:
                if self.positive:
                    ...
                    return wj
        if wj > 0:
            return max(0, wj * self.mcp_k - self.mcp_b * v)
        else:
            if self.positive:
                return 0
            else:
                return min(0, wj * self.mcp_k + self.mcp_b * v)
